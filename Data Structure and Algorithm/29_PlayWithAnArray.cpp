...
int main()
{
    ...
    bool isValid = true;
    for (int i = 1; i < n - 1; i += 2) { // Loop through odd indices
        if (result[i] <= result[i - 1]) { // Check if the odd-indexed element is less than or equal to its even-indexed neighbor
            isValid = false;
            break;
        }
    }
